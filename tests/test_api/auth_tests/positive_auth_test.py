...
class TestPositiveAuthAPI:
    @allure.story("Register user from PUBLIC role")
    ...
    def test_register_user(self, api_manager, auth_user_data):
        ...
        with allure.step("Чистим тестового пользователя"):
            api_manager.user_api.clean_up_user(response_data["id"])

    @allure.story("Register and login new user")
    @pytest.mark.positive
    @pytest.mark.auth
    ...
    @allure.story("Login and check token structure")
    @pytest.mark.positive
    @pytest.mark.auth

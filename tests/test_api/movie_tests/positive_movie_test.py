...
class TestPositiveMoviesAPI:
    ...
    def test_patch_movie_by_superadmin(self, created_movie_with_delete, authorized_admin, movie_data):
        ...
        with allure.step("Проверяем, что данные изменились после PATCH"):
    ...
    @allure.story("DELETE movie as SUPER_ADMIN")
    @pytest.mark.positive
    @pytest.mark.superadmin
    ...
    @allure.story("Create movie with min boundary values")
    @pytest.mark.positive
    @pytest.mark.superadmin
